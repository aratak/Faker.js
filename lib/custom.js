...
module.exports = function(options) {
  ...
  result['addToCollection'] = function(collectionName, newElement) {
    options.collections[collectionName].push(newElement);
  };
  ...
  result['collections'] = function(collectionName, newElement) {
    ...
    if (arguments.length == 1) {
      return {
        'add': function(newElement) { addNewElement(collectionName, newElement); }
      };
    } else {
      return addNewElement(collectionName, newElement);
    }

    return function(newElement) { addNewElement(collectionName, newElement); };
  }
